...
- **Contextual Action Bar**: A floating "Glass" bar at the top-right of the focused pane with:
  ...
  - `Close Pane`
- **Interactive Resizing**: A center "Handle" on every split for manual dragging with a ghost-layout preview.
...
- **Backend (Tauri/Rust)**: 
  - Enhance `WmuxCore` to support proportional split ratios (ratios between 0.0 and 1.0).
  - Add process monitoring to `wmux-core` for status bar intelligence.
  - Implement "Layout Transitions" by interpolating split ratios over time.
